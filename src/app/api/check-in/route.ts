...
const calculatePoints = (streak: number) => {
  ...
  let bonusPoints = 0;
  if (streak >= 30) bonusPoints = 50000;      // Monthly milestone
  else if (streak >= 7) bonusPoints = 2000;  // Weekly milestone
  else if (streak >= 3) bonusPoints = 500;   // Mini streak
  
  return basePoints + bonusPoints;
};

// GET endpoint to fetch check-in data
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (user.lastCheckIn && isSameDay(new Date(user.lastCheckIn), new Date())) {
      return NextResponse.json(
    ...
    user.points += pointsEarned; // Add to total points
